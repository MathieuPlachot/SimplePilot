...
class ConnectionStatusWidget extends StatelessWidget {
  ...
  const ConnectionStatusWidget({super.key, this.bulletDiameter = 15.0});

  @override
  Widget build(BuildContext context) {
    final status = context.watch<UDPHandler>().connectionStatus;

    // Map enum → display text
    final String textValue = (status == ConnectionStatus.connected)
        ? "Connected"
        : "No connection";

    // Map enum → bullet color
    final Color bulletColor = (status == ConnectionStatus.connected)
        ? AppColors.success
        : AppColors.danger;
